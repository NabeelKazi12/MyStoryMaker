...
| D-25 | Los personajes se declaran en el encargo (`PersonajeDeclarado`) y la apertura los cobra: el Planner tiene que proponerlos todos, con su nombre y su papel, y hacerlos salir en algún evento o escena, o el plan se rechaza entero | Escribirlos en el canon al cerrar la entrevista: salta la apertura y convierte el encargo en una canonización, contra `AGENTS.md` §1.5. Dejarlos como sugerencia del prompt: es lo que ya pasaba con el destinatario, que el prompt pedía y nada comprobaba | §6 |
| D-26 | El cambio de nombre de un personaje es una sustitución exacta, sin modelo, en canon, esqueleto, resúmenes y prosa de su novela, en una sola transacción: revisión de canon con eventos `renombrar`, `Borrador` nuevo por escena tocada con el anterior `obsoleto` y versión nueva con los capítulos marcados | Regenerar los capítulos con el Redactor: cuesta invocaciones y reescribe lo que nadie pidió cambiar. Cambiar solo la ficha: deja canon fantasma, porque los resúmenes y la prosa siguen con el nombre viejo y el capítulo siguiente lo recupera | §3.3 |
| D-24 | Cada llamada al modelo es una traza de Langfuse con el id de su `Procedencia`, en la sesión de su novela; los gastos se leen de SQLite. El transporte HTTP lo inyecta el worker, porque `observability/` y `api/` no pueden hacer red, y el enlace a la traza se compone con `LANGFUSE_PROJECT_ID` | Una traza por novela con un span por llamada: no enlaza cada fila de la pestaña con su llamada. Leer los gastos de Langfuse: la pestaña dependería de un servicio externo, contra la decisión de SPEC-003 de que SQLite es la fuente de verdad | §6 |
| D-23 | Eliminar una novela la **retira** con fecha (`volumen.eliminada_en`): desaparece de la biblioteca y todas las rutas con `{volumen_id}` responden `404`, por una dependencia común que un test comprueba sobre todas las rutas. Restaurar es poner la columna a `NULL` en la base; la interfaz no lo ofrece | Borrado en cascada de capítulos, escenas, borradores, canon, versiones y tareas: rompe `VersionAnteriorSeConserva`, el trigger que impide borrar aprobaciones (D-22) y la inmutabilidad de `audit_log` | §6 |
| D-22 | La firma humana (`AprobacionDeVolumen`) es la evidencia de *promesa al lector* en *Volumen cerrado*; lo determinista de la puerta sigue bloqueando con firma, y aprobar bloquea reescribir, cambiar y retitular hasta reabrir | Aceptar por excepción autorizada cada borrador al aprobar: salta *Escena limpia*, que es el riesgo F-06 de `verification.md`. Una marca de aprobación que no toca ninguna puerta: deja *Volumen cerrado* sin evaluar para siempre | §6 |
